...
function initSkills(){
  ...
  const cards=Array.from(document.querySelectorAll('.skill-card, .skill-strip'));
...
function initSkillCharts(){
  ...
  const CAT_LABELS={frontend:'Frontend',backend:'Backend',ai:'AI / ML',databases:'Databases',cloud:'Cloud',devops:'DevOps',methodologies:'Practices'};
  charts.forEach(c=>{
...
/* ── 15. SKILL → PROJECT FILTER ── */
function initSkillFilter(){
  const searchInput = document.getElementById('projectSearch');
  if(!searchInput) return;
  const pills = Array.from(document.querySelectorAll('#skills [data-search]'));
  if(!pills.length) return;

  pills.forEach(pill=>{
    pill.addEventListener('click',()=>{
      const term = pill.dataset.search;
      const isActive = pill.classList.contains('skill-pill--active');

      // Clear all active states first
      pills.forEach(p=>p.classList.remove('skill-pill--active'));

      if(isActive){
        // Second click on same pill — clear the filter
        searchInput.value = '';
      } else {
        searchInput.value = term;
        pill.classList.add('skill-pill--active');
      }
      // Reuse existing filter logic — just fire the input event
      searchInput.dispatchEvent(new Event('input',{bubbles:true}));

      // Scroll to projects after a brief delay so the filter applies first
      setTimeout(()=>{
  ...
  // When the user manually clears the search box, also clear active pill state
  searchInput.addEventListener('input',()=>{
    if(!searchInput.value.trim()) pills.forEach(p=>p.classList.remove('skill-pill--active'));
  });
}
...
function initCopyEmail(){
  ...
  // Assemble from data attributes — keeps plain address out of HTML source
  const email=(emailSpan.dataset.u||'')+'@'+(emailSpan.dataset.d||'');
  emailSpan.textContent=email;

  // Wire error-alert fallback link
  const errLink=document.getElementById('contactErrorLink');
  if(errLink) errLink.href='mailto:'+email;
  ...
  const doCopy=async()=>{
    ...
    } catch{ /* clipboard unavailable — address is visible on screen */ }
  };
